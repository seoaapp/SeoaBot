    ...
    if (query.args[0]) {
      if (['lang', '언어'].includes(query.args[0].toLowerCase())) {
        if (!(query.args[1] === 'kor' || query.args[1] === 'en' || query.args[1] === 'pt')) {
          ...
          msg.channel.send(i18n.__({phrase: 'ENMSG', locale: settings.servers[msg.guild.id].lang}))
        } else {
          settings.servers[msg.guild.id].lang = query.args[1]
          fs.writeFileSync('./ServerData/servers.json', JSON.stringify(settings.servers, null, '  '))
          msg.channel.send(i18n.__({phrase: 'Lang', locale: settings.servers[msg.guild.id].lang}, query.args[1]))
        }
        ...
        fs.writeFileSync('./ServerData/servers.json', JSON.stringify(settings.servers, null, '  '))
        msg.channel.send(i18n.__({phrase: 'su', locale: settings.servers[msg.guild.id].lang}))
      }
    } else {
      msg.channel.send(i18n.__({phrase: 'ch', locale: settings.servers[msg.guild.id].lang}))
    }
  } else {
    msg.channel.send(i18n.__({phrase: 'MISS', locale: settings.servers[msg.guild.id].lang}))
  }
}
