...
module.exports = class extends Model {
  constructor () {
    super(['m', 'music'], 'music', 'music')
  }

  async run (msg, args, p) {
    args.splice(0, 1)
    const embed = new p.Embed()
    const policy = msg.client.config.localPolicy.music
    if (!msg.client.m.servers.has(msg.guild.id)) {
      msg.client.m.once(`${msg.guild.id}_add`, _here => {
    ...
    if (args[0] === 'join') here._(msg.member.voiceChannel)
      ...
      const search = new Search(p.client.config.api.youtube)
        .addParam('type', 'video')
        .addParam('order', 'relevance')
      if (!args[1]) return
      search._(args.splice(1).join(' '), 5).then(res => {
      ...
      const mylist = await msg.client.db.select('mylist', { name: args[1] })
      here.mylist(mylist[0])
    } else {
      const desc = '**[] Optinal, <> Required**\n(p)m <join|leave|stop|skip>\n(p)m <now|queue>\n(p)m <play|repeat|fix>\n(p)m search <query>\n(p)m url <youtube_url>\n(p)m vol <percentage>\n(p)m mylist <mylist_ID>'
      msg.channel.send(desc.split('(p)').join(msg.client.config.prefix))
    }
  }
}
